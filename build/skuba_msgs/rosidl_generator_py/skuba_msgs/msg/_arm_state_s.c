...
bool skuba_msgs__msg__arm_state__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("skuba_msgs.msg._arm_state.ArmState", full_classname_dest, 34) == 0);
  }
  skuba_msgs__msg__ArmState * ros_message = _ros_message;
  {  // positions
  ...
  {  // efforts
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      float * dest = ros_message->efforts.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
...
PyObject * skuba_msgs__msg__arm_state__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("skuba_msgs.msg._arm_state");
  ...
  {  // efforts
    ...
    if (ros_message->efforts.size > 0) {
      // populating the array.array using the frombytes method
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      float * src = &(ros_message->efforts.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->efforts.size * sizeof(float));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
